    ...
    <div className="flex flex-col h-full bg-neutral-950">
      ...
      <div className="relative w-full aspect-[9/16] bg-black max-h-[50vh] flex-shrink-0 mx-auto max-w-[360px] overflow-hidden rounded-b-2xl border-b border-neutral-800 shadow-2xl">
        ...
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
          <div className="bg-red-600/90 text-white px-4 py-2 rounded-full font-bold text-sm tracking-widest uppercase shadow-[0_0_20px_rgba(239,68,68,0.5)] flex items-center gap-2">
            <span className="w-2 h-2 rounded-full bg-white animate-pulse"></span>
            Playing in Main Player
          </div>
        </div>
        {/* Overlay to block interaction and show topic */}
        <div className="absolute top-4 left-4 right-4 z-10 text-white shadow-md rounded-lg overflow-hidden backdrop-blur-md bg-black/60 border border-white/10 p-3">
            <p className="text-[10px] text-red-400 font-bold uppercase tracking-widest mb-1">Playing Segment {currentIndex + 1} of {playlist.length}</p>
            <p className="text-sm font-semibold leading-tight">{currentClip.topic}</p>
        </div>
      </div>
      
      {/* Playlist Area */}
      <div className="flex-1 overflow-y-auto p-4 custom-scrollbar">
        <h3 className="text-xs font-bold text-neutral-500 uppercase tracking-widest mb-4 px-1">Highlight Reel</h3>
        <div className="space-y-2">
          {playlist.map((clip: any, i: number) => (
            <div 
              key={i} 
              onClick={() => setIndex(i)}
              className={`p-3 rounded-xl border transition-all cursor-pointer flex flex-col gap-1.5 ${currentIndex === i ? 'bg-red-500/10 border-red-500/50 shadow-[0_0_15px_rgba(239,68,68,0.1)]' : 'bg-neutral-900 border-neutral-800 hover:border-neutral-600'}`}
            >
              <div className="flex items-center justify-between">
                ...
                {currentIndex === i && <span className="flex w-2 h-2 rounded-full bg-red-500 animate-pulse shadow-[0_0_8px_rgba(239,68,68,0.8)]"></span>}
              </div>
              <p className={`text-[13px] leading-snug ${currentIndex === i ? 'text-white font-semibold' : 'text-neutral-400'}`}>{clip.topic}</p>
            </div>
          ))}
        </div>
      </div>
          ...
          <div className="flex items-start mb-2">
            <div className="bg-transparent px-4 py-2 flex items-center gap-3">
              {searchQuery ? (
                ...
                <div className="flex gap-1.5 items-center justify-center p-2">
...
const MessageCopyButton = ({ text }: { text: string }) => {
  ...
  return (
    <button 
      onClick={handleCopy}
      className="absolute top-2 right-2 p-1.5 bg-neutral-800/80 backdrop-blur-sm border border-neutral-700/50 text-neutral-400 hover:text-white hover:bg-neutral-700 rounded-lg opacity-0 group-hover/msg:opacity-100 transition-all shadow-md flex items-center justify-center z-10"
      title="Copy message"
    >
      {copied ? <Check size={14} className="text-green-500" /> : <Copy size={14} />}
    </button>
